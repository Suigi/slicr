...
describe('useAppActions', () => {
  ...
  it('ignores slice node selection while layout is unsettled', () => {
    ...
    actions.onNodeSelect('node-a');

    expect(setSelectedNodeKey).not.toHaveBeenCalled();
  });

  it('ignores slice hover highlighting while layout is unsettled', () => {
    const setHighlightRange = vi.fn();
    const actions = useAppActions({
      diagramMode: 'slice',
      layoutReady: false,
    ...
    actions.onNodeHoverRange({ from: 10, to: 20 });

    expect(setHighlightRange).not.toHaveBeenCalled();
  });
});
