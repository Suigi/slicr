...
export type ElkEdgeGeometry = { d: string; labelX: number; labelY: number; points?: ElkPoint[] };

export type ElkComputedLayout = {
  pos: Record<string, Position>;
  w: number;
  h: number;
  edges: Record<string, ElkEdgeGeometry>;
  laneByKey: Map<string, number>;
  rowStreamLabels: Record<number, string>;
};

export function routePolyline(points: ElkPoint[]): string {
  if (points.length === 0) {
    return '';
  }
...
export function middlePoint(points: ElkPoint[]): ElkPoint {
  if (points.length === 0) {
    return { x: 0, y: 0 };
  }
...
export async function computeElkLayout(parsed: Parsed): Promise<ElkComputedLayout> {
  ...
  const edges: Record<string, ElkEdgeGeometry> = {};
